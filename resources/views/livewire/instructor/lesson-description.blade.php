                ...
                <hr class="my-2">
                    @if ($lesson->description)
                        <form wire:submit.prevent="update">
                            <textarea wire:model="description.name"  class="form-input w-full" placeholder="Agregue una descripción de la lección..."></textarea>
                    ...
                    @else 
                            <div>
                                <textarea wire:model="name"  class="form-input w-full "></textarea>
                                @error('name')
                                    <span class="text-sm text-red-500">{{$message}}</span>
                                @enderror
